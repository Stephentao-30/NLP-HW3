...
def hybrid_search(query, faiss_index, bm25, chunks, embedder, url_groups,
    ...
    # SentenceTransformer encode is not reliably thread-safe across environments.
    # Serialize encode calls to avoid intermittent crashes/semaphore leaks.
    with _embedder_lock:
...
def main():
    ...
    with open(questions_path, 'r', encoding='utf-8') as f:
        # Keep one question per input line; preserve internal spacing.
        questions = [line.rstrip("\n") for line in f]

    fast_model = ALLOWED_MODELS[0]

    def fetch_answer(i, q):
        ...
        try:
            ...
            return i, clean, context
        except Exception as e:
            return i, "Unsure about answer", ""

    start_time = time.time()

    answers = ["Unsure about answer"] * len(questions)
    contexts_saved = [""] * len(questions)

    workers = min(10, max(1, len(questions)))
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        futures = [executor.submit(fetch_answer, i, q) for i, q in enumerate(questions)]
        for future in concurrent.futures.as_completed(futures):
            i, ans, ctx = future.result()
            answers[i] = ans
            contexts_saved[i] = ctx
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        for ans in answers:
            ...
            f.write(f"{ans.replace(chr(10), ' ').replace(chr(13), ' ')}\n")
